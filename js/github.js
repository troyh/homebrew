	...
	$.getJSON("https://api.github.com/repos/"+repo.user+"/"+repo.repo+"/git/refs/heads/"+repo.branch,null,function(ref) {
		...
		$.getJSON("https://api.github.com/repos/"+repo.user+"/"+repo.repo+"/git/trees/"+ref.object.sha,null,function(tree) {
			...
			var postdata=				{
				...
				console.log(postdata);
			$.post("https://api.github.com/repos/"+repo.user+"/"+repo.repo+"/git/trees",
				postdata,
				function(new_tree) {
					console.log("trees response:");
					console.log(new_tree);
					$.post("https://api.github.com/repos/"+repo+"/"+repo.repo+"/git/commits",
						{
							message: message,
							tree: new_tree.sha,
						...
						function(commit) {
							...
							console.log(commit);
							$.post("https://api.github.com/repos/"+repo+"/"+repo.repo+"/git/refs/"+ref.object.sha,
								{
										sha: commit.sha
								},
								function(ref) {
